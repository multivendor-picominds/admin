...
@php
    $payable_amount = $total_price * $flutterwave->currency->currency_rate;
    ...
    function makePayment() {
      FlutterwaveCheckout({
        ...
        callback: function (data) {
            ...
            let coupon = "{{ $coupon }}";
            let token = "{{ $token }}";
            let order_id = "{{ $find_order->order_id }}";
            $.ajax({
                type: 'post',
                data : {tnx_id, _token, shipping_address_id, billing_address_id, shipping_method_id, coupon, token, order_id},
                url: "{{ route('user.checkout.guest.pay-with-flutterwave') }}",
