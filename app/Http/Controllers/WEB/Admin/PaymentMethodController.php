...
class PaymentMethodController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin.payment_method', compact('paypal','stripe','razorpay','bank','paystackAndMollie','flutterwave','instamojo','countires','currencies','setting','paymongo','sslcommerz','myfatoorah','baksh'));
    ...
    public function updateBaksh(Request $request){
        ...
        $customMessages = [
            'account_mode.required' => trans('admin_validation.Account Mode id is required'),
            'username.required' => trans('admin_validation.User Name id is required'),
            'password.required' => trans('admin_validation.Password id is required'),
            'app_key.required' => trans('admin_validation.App Key id is required'),
            'app_secret.required' => trans('admin_validation.App Secret id is required'),
            'currency_name.required' => trans('admin_validation.Currency name is required'),
        ];
        ...
        $baksh = BakshPayment::first();
        if($request->image){
            $old_image=$baksh->image;
            $image=$request->image;
            $extention=$image->getClientOriginalExtension();
            $image_name= 'bkash-'.date('Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            $image_name='uploads/website-images/'.$image_name;
            Image::make($image)
                ->save(public_path().'/'.$image_name);
            $baksh->image=$image_name;
            $baksh->save();
            if(File::exists(public_path().'/'.$old_image))unlink(public_path().'/'.$old_image);
        }

        $baksh->account_mode = $request->account_mode;
        $baksh->username = $request->username;
        $baksh->password = $request->password;
        $baksh->app_key = $request->app_key;
        $baksh->app_secret = $request->app_secret;
        $baksh->currency_id = $request->currency_name;
        $baksh->status = $request->status ? 1 : 0;
        $baksh->save();
