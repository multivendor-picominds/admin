...
class DeliveryManProfileController extends Controller
{
    ...
    public function UpdateDeliveryManLatlong(Request $request){
        $rules = [
            'latitude'=>'required',
            'longitude'=>'required',
        ];
        $customMessages = [
            'latitude.required' => trans('user_validation.latitude is required'),
            'longitude.required' => trans('user_validation.longitude is required'),
        ];
        $this->validate($request, $rules,$customMessages);

        $deliveryman=Auth::guard('deliveryman-api')->user();
        $deliveryman->latitude = $request->latitude;
        $deliveryman->longitude = $request->longitude;
        $deliveryman->save();

        $notification = trans('user_validation.Latitude & Longitude Updated');
