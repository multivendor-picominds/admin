...
class DeliveryManOrderController extends Controller
{
    public function index(){
        ...
        $orders = Order::with('user','deliveryman')->where('delivery_man_id', $deliveryman_id)->where('order_request', 1)->get();
    ...
    public function orderRequest(){
        $deliveryman_id=Auth::guard('deliveryman-api')->user()->id;
        $orders = Order::with('user','deliveryman')->where('delivery_man_id', $deliveryman_id)->where('order_request', '=', 0)->orderBy('id','desc')->get();
        $setting = Setting::first();
        return response()->json([
            'orders' => $orders,
            'setting' => $setting,
    ...
    public function completedOrder(){
        $deliveryman_id=Auth::guard('deliveryman-api')->user()->id;
        $orders = Order::with('user','deliveryman')->where('delivery_man_id', $deliveryman_id)->where('order_status', '=', 3)->where('order_request', '=', 3)->orderBy('id','desc')->get();
