...

{
    ...
    public function websiteSetup(Request $request){
        ...
        if($request->lang_code){
            ...
            $default_language = Language::where('lang_code','en')->first();
        }

        $currencies = MultiCurrency::where('status',1)->orderBy('currency_name','asc')->get();

        $setting = Setting::with('currency')->select('currency_id','logo','favicon','enable_user_register','phone_number_required','default_phone_code','enable_multivendor','text_direction','timezone','topbar_phone','topbar_email','currency_icon','currency_name','show_product_progressbar','theme_one','theme_two','seller_condition','map_status','map_key')->first();
    ...
    public function blogComment(Request $request){
        ...
        $comment = BlogComment::where('id', $comment->id)->first();

        $notification = trans('user_validation.Blog comment submited successfully');



        return response()->json(['message' => $notification, 'comment' => $comment],200);
